...
- Página de Separator: `589:983` (set `590:3436`; horizontal de 193 × 1 px y vertical de 1 × 69 px)
- Página de Slider: `2772:1030` (set `2777:58`; tipos Single, Range y Multiple, orientación horizontal/vertical, estados Default/Disabled y valores Low/Medium/High)
- Página de Input OTP: `561:4282` (Base `566:5307`, Separator `570:786`; slots 4–6, estados Default/Error/Disabled y pasos Empty/Selected/Partial/Filled)
- Página de Data Table: `2366:19504` (composición Toolbar + Table + Pagination; sorting, filtering, visibilidad, selección y paginación mediante TanStack Table v9)
...
Navigation Menu conserva estrictamente la implementación oficial vigente de shadcn/ui sobre Radix: Root con `viewport`, List, Item, Trigger, Content, Link, Viewport, Indicator y `navigationMenuTriggerStyle`. El nodo `2997:803` coincide con la geometría oficial: trigger de 36 px, padding `16px 8px`, chevron de 12 px y gap de 4 px. Los contenidos de Figma se resuelven por composición en `NavigationMenuExample`: `list` de 400 × 268 px y `featured` de 520 × 196 px, ambos mediante links públicos reales de Navigation Menu. Cantidad de 1–10, apertura inicial, ítem activo, último ítem dropdown/link y disabled son argumentos exclusivos del ejemplo; no amplían el primitive. `viewport=false` usa el comportamiento y posicionamiento oficial, no un popover paralelo. `NavigationMenuIndicator` permanece exportado como parte de la API oficial opcional, pero la instancia GRM no lo renderiza porque el nodo de Figma no incluye el rombo indicador. Docs y Playground renderizan la misma instancia compartida.

Separator conserva sin extensiones la implementación oficial de shadcn/ui sobre Radix: `orientation="horizontal" | "vertical"`, `decorative=true` por defecto, `data-orientation` y estilos mediante `className`. El nodo `590:3436` aporta únicamente las longitudes de referencia, 193 px horizontal y 69 px vertical; estas dimensiones se aplican en el contenedor de `SeparatorExample` y no se convierten en props del primitive. El grosor permanece en 1 px y el color consume `--border` para responder a la marca activa. Docs y el único Playground renderizan la misma instancia compartida.

Slider conserva literalmente el wrapper oficial `radix-nova` de shadcn/ui sobre Radix: Root, Track, Range y un Thumb por cada elemento de `value` o `defaultValue`. Single, Range y Multiple no son variantes privadas: se componen con arreglos de uno, dos o tres valores. El nodo `2777:58` aporta la geometría de referencia de 240 × 20 px horizontal y 20 × 240 px vertical, track de 4 px, estados Default/Disabled y presets Low/Medium/High; estas decisiones pertenecen exclusivamente a `SliderExample`. Range integra la receta Controlled oficial mediante el Label público, estado React, `value`, `onValueChange` y valor visible; no existe una variante Controlled separada y el label no se añade al primitive. Orientación, límites, step, disabled, teclado, gestos, foco y ARIA permanecen en la API pública oficial. Docs y el único Playground consumen `sliderExamplePresets` y la misma instancia `SliderExample`.

Input OTP conserva la composición pública oficial `radix-nova` de shadcn/ui sobre `input-otp`: `InputOTP`, `InputOTPGroup`, `InputOTPSlot` e `InputOTPSeparator`, junto con `OTPInputContext`, caret, teclado, paste, pattern, disabled y ARIA. Figma adapta cada slot a 36 × 36 px, fondo `--card`, radio de 8 px y focus ring de 3 px; no sustituye la entrada accesible. Cantidad de 4 a 6 slots, estilo continuo o separado, distribución 3-3/2-2-2, estados Default/Error/Disabled y pasos Empty/Selected/Partial/Filled pertenecen exclusivamente a `InputOTPExample`. Error mantiene `aria-invalid` en la entrada real y lo propaga a los slots públicos para que borde y focus ring consuman los tokens destructivos incluso cuando el input oculto es hermano de los grupos en el DOM generado por `input-otp`. La composición usa `REGEXP_ONLY_DIGITS` y crea grupos y separadores públicos reales. Docs y el único Playground consumen `inputOTPExamplePresets` y la misma instancia `InputOTPExample`.
...
El nodo `3114:1373` define anchos de 256 px expandido y 64 px colapsado, header de 56 px, ítems de navegación de 240 × 40 px en expanded y 48 × 40 px en collapsed, iconos de 16 px, padding horizontal de 12 px y grupos separados por 20 px. Los labels usan 11 px, uppercase y tracking de `0.1em`; en collapsed ocultan el texto y conservan un divisor de 24 px. La subnavegación colapsada ocupa 48 × 90 px y centra ítems de 40 × 32 px con iconos de 14 px. Su contenedor abierto mide 134 px: trigger de 40 px, panel de 90 px y padding inferior de 4 px, aplicado únicamente en la combinación `collapsed + open`. Los ítems colapsables mantienen el chevron a la derecha, centrado verticalmente, y cada opción icon-only conserva su tooltip. El trigger circular conserva 32 px y representa la transición mediante chevron izquierdo en `expanded` y derecho en `collapsed`.
...
Checkbox y Switch se componen con los componentes públicos del sistema. Chevron controla una región expandida accesible mediante `aria-expanded`; la fila cerrada mide 49 px y la referencia abierta conserva altura intrínseca. La alternancia visual de filas se controla en TableBody y consume `--muted` al 5 %.

El header usa padding horizontal de 10 px, texto de 12/16, peso semibold, uppercase, `--muted-foreground` e icono de orden de 12 px. Las celdas usan padding horizontal de 10 px, vertical de 8 px y texto base de 14/20. TableCell no puede imponer tipografía a sus descendientes: Badge, Avatar, Button, links, identificadores y textos secundarios conservan su API visual propia. Docs y Playground renderizan `TableExample`; no se recrean tablas documentales para representar el componente.

Los encabezados ordenables de Data Table reutilizan Button ghost exclusivamente como control semántico. Su expresión debe coincidir con Header CellContent/Button de Figma: altura interna de 16 px, texto 12/16 semibold uppercase, gap de 10 px, icono de 12 px, sin padding interno adicional y sin superficie de fondo en hover/active. El padding de 10 px permanece en TableHead.

`_components/Table/TableCell/CellContent` se traduce a composiciones públicas independientes de TableCell: `TableCellText`, `TableCellAvatar`, `TableCellStatus`, `TableCellProgress`, `TableCellCounter` y `TableCellBulkOptions`. Text admite una/dos líneas, Sans/Mono y énfasis regular/medium/link. Los valores numéricos se componen con `TableCellText font="mono"`, consumen `--brand-font-mono` y se alinean a la derecha; la tabla canónica incluye una columna Valor que documenta este formato. Avatar admite Supporting, Supporting Compact, Metadata y Custom. Status Label usa indicador de 6 px más etiqueta y tonos success/warning/destructive. Status Badge es una composición alternativa mediante el Badge público `appearance="outline"` y `size="xl"`; no se incorpora como variante interna de `TableCellStatus`. Progress redondea a incrementos de 10 y usa destructive por debajo de 80 %, warning entre 80–89 % y success desde 90 %. Counter opera entre 1 y 10 mediante Button Group. Bulk Options compone Icon Buttons con gap de 6 px.

Checkbox, Switch y Chevron no pertenecen a CellContent: son `Leading Control` de TableRow. El padding, ancho y truncamiento continúan siendo responsabilidad de TableCell; las composiciones internas no deben imponerlos.

### Caso Data Table

El nodo `2366:19504` admite dos disposiciones independientes para Toolbar y Pagination. En `attached`, cada región se une visualmente a Table mediante una superficie con borde: Toolbar tiene 62 px mínimos y padding de 12 px; Pagination tiene 56 px mínimos y padding de 12 px. En `separated`, ambas quedan a 12 px de Table y no añaden fondo, borde ni padding contenedor: Toolbar conserva 36 px y Pagination 32 px. La paginación mantiene texto de página y cuatro controles de 32 px. Estas regiones reutilizan Input Group, Button, Dropdown Menu, Select, Switch y el primitive Table público del sistema.

Data Table sigue la guía oficial de shadcn/ui: no reemplaza Table ni introduce un primitive visual independiente. La lógica headless se implementa con TanStack Table v9 mediante `useTable`, `tableFeatures` y `table.FlexRender`; registra filtering, sorting, visibility, row selection, row expansion y pagination con sus row models correspondientes. Solo se registran `filterFn_includesString`, `sortFn_alphanumeric` y `sortFn_text`; no se incorporan los catálogos completos. Columns y data permanecen como entradas de composición.

`DataTableExample` es la instancia canónica compartida por el único Playground y todas las muestras de Docs. Sorting vive en los encabezados, filtering en el toolbar, visibility en Dropdown Menu, selection en Checkbox y pagination en el footer. Las acciones por fila admiten `none`, el Dropdown Menu oficial de shadcn/ui o `TableCellBulkOptions` con uno, dos o tres Icon Buttons públicos; las muestras distribuyen tres acciones en Checkbox, dos en Switch, una en Identificador y Dropdown en Expandible. Los cuatro patrones de Figma —checkbox, switch, identificador y chevron expandible— son composiciones de columnas, no variantes del primitive Table. Las celdas compuestas reutilizan `TableCellAvatar`, `TableCellStatus` y `TableCellBulkOptions`; Badge solo se usa cuando el producto pide explícitamente esa composición.

Data Table admite `loading`, `empty` y `error`, panel de detalle mediante `renderSubComponent` y opciones controladas mediante `tableOptions`. La paginación remota debe combinar `manualPagination`, una página de datos ya procesada, `rowCount` o `pageCount`, `state.pagination`, `onPaginationChange` y `autoResetPageIndex=false`. Toolbar y footer permanecen slots externos porque cada tabla de producto es única. Docs no reconstruye filas, toolbar ni controles con HTML paralelo.
