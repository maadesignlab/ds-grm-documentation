...
function Card({ title, value, children, tall = false }: { title: string; value: string; children: React.ReactNode; tall?: boolean }) { return <article className="overflow-visible rounded-lg border border-border bg-card"><div className={`sb-unstyled flex items-start justify-center overflow-visible rounded-t-lg bg-background p-8 ${tall ? "min-h-[360px]" : "min-h-40"}`}>{children}</div><div className="flex min-h-14 items-center justify-between gap-3 border-t border-border px-4"><strong className="truncate text-card-foreground text-sm leading-5">{title}</strong><Code>{value}</Code></div></article> }
...
export function NavigationMenuSpecifications() {
  return <div className="not-prose grid gap-6"><section><h3 className="mb-3 text-base leading-6 font-semibold text-foreground">API y anatomía</h3><Table columns={["Parte", "Primitive", "API", "Función"]} rows={api} /></section><section><h3 className="mb-3 text-base leading-6 font-semibold text-foreground">Tamaño y espaciado</h3><Table columns={["Parte", "Tailwind", "Medida", "Espaciado", "Radio"]} rows={geometry} /></section><section><h3 className="mb-3 text-base leading-6 font-semibold text-foreground">Tipografía y color</h3><Table columns={["Elemento", "Tailwind", "Valor", "Token"]} rows={typography} /></section></div>
}
