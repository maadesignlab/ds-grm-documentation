...
<summary><strong>v1.0.1</strong> · Sincronización Figma para DS v1.1.0</summary>
  ...
  **Tokens consumidos en esta actualización:** `border`, `table/row-alternate`, `input`, `background`, `primary/5%`, `primary`, `foreground`, `muted/foreground`, `primary/10%`, `primary/default-foreground`, `warning/light-foreground`, `success`, `destructive`, `muted`, `table/header/background`. Consulta sus valores anteriores y actuales en Foundations → Tokens.
