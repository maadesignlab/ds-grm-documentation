    ...
    <section className="not-prose overflow-hidden rounded-lg border border-border bg-card">
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", minWidth: 780, borderCollapse: "collapse", tableLayout: "fixed" }}>
          ...
          <tbody>{specifications.map((row, rowIndex) => <tr key={row[0]} style={{ background: rowIndex % 2 ? "color-mix(in srgb, var(--muted) 30%, transparent)" : "transparent" }}>{row.map((value, columnIndex) => <td key={columns[columnIndex]} style={{ height: 52, padding: "10px 16px", border: 0, borderBottom: rowIndex === specifications.length - 1 ? 0 : "1px solid var(--border)", color: "var(--foreground)", fontSize: 12, lineHeight: 1.4, verticalAlign: "middle" }}>{columnIndex === 0 ? <Code>{value}</Code> : value.includes("·") ? <Code>{value}</Code> : value}</td>)}</tr>)}</tbody>
        </table>
      </div>
    </section>
  )
...
export function AttachmentTokenSpecifications() {
  ...
  return (
    <section className="not-prose overflow-hidden rounded-lg border border-border bg-card">
      <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}><thead><tr style={{ background: "var(--muted)" }}>{["Parte", "Propiedad", "Token / utilidad"].map((column) => <th key={column} style={{ padding: "11px 16px", border: 0, borderBottom: "1px solid var(--border)", color: "var(--muted-foreground)", fontSize: 11, fontWeight: 600, letterSpacing: "0.04em", lineHeight: 1.4, textAlign: "left", textTransform: "uppercase", verticalAlign: "middle" }}>{column}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={row.join()} style={{ background: index % 2 ? "color-mix(in srgb, var(--muted) 30%, transparent)" : "transparent" }}>{row.map((value, column) => <td key={value} style={{ height: 52, padding: "10px 16px", border: 0, borderBottom: index === rows.length - 1 ? 0 : "1px solid var(--border)", color: "var(--foreground)", fontSize: 12, lineHeight: 1.4, verticalAlign: "middle" }}>{column === 0 || column === 2 ? <Code>{value}</Code> : value}</td>)}</tr>)}</tbody></table>
    </section>
  )
}
